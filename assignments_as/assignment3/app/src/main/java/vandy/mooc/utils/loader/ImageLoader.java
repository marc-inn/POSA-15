...
public class ImageLoader
        implements GenericAsyncTaskOps<ImageLoaderWorkOrder, Void, ImageLoaderWorkResult> {
    /**
     * Logcat Tag
     */
     ...
     */
    private final Executor mDisplayThreadPoolExecutor = ImageLoaderThreadPool.MY_THREAD_POOL_EXECUTOR;
     ...
     * for it.
     */
    private final Map<Integer, String> mCacheKeysForImageView = new ConcurrentHashMap<>();
     ...
     */
    private void addBitmapToCache(String filename, Bitmap bitmap) {
     ...
     * efficient.
     */
    public void loadAndDisplayImage(ImageView view, String imageFilePath, int colWidth) {
        // Store the holder and its current filepath in the cache
        mCacheKeysForImageView.put(view.hashCode(), imageFilePath);

        Bitmap cachedBitmap = mBitmapCache.get(imageFilePath);
        ...
        if (cachedBitmap != null)
            ...
            ImageLoaderWorkOrder wo = new ImageLoaderWorkOrder(new ImageViewHolder(view),
                    imageFilePath,
                    colWidth,
                    colWidth);

            // Create a new GenericAsyncTask
            GenericAsyncTask<ImageLoaderWorkOrder, Void, ImageLoaderWorkResult, ImageLoader> imageLoaderTask = new GenericAsyncTask<>(
                    this);

            // Use the task to load the image in the background.
            imageLoaderTask.executeOnExecutor(mDisplayThreadPoolExecutor, wo);
        }
    }
     ...
     * image
     */
    private void checkImageView(ImageViewHolder imgView, String filepath)
            throws ViewChangedException {
        checkViewCollected(imgView);
        checkViewReused(imgView, filepath);
    }
     ...
     */
    private void checkViewCollected(ImageViewHolder imgView) throws ViewChangedException {
        if (imgView.isCollected()) throw new ViewChangedException();
    }
     ...
     * a different image.
     */
    private void checkViewReused(ImageViewHolder imgView, String filepath)
            throws ViewChangedException {
        if (isViewReused(imgView, filepath)) throw new ViewChangedException();
    }
     ...
     */
    private boolean isViewReused(ImageViewHolder imgView, String filepath) {
    ...
    public ImageLoaderWorkResult doInBackground(ImageLoaderWorkOrder... param) {
        ...
        try {
            ...
            // Load the bitmap
            result = BitmapUtils.decodeSampledBitmapFromFile(filepath, width, height);
